...
const EventPage = async ({ params }: { params: Promise<{ slug: string }> }) => {
  ...
  const EventDetails = ({ image, alt, info }: { image: string, alt: string, info: string }) => (
  ...
  const events = await similarEvents(slug);
  return (
    <section id="event">
      ...
      <div className="details">
        ...
        <Image src={image} alt={title} width={800} height={800} className="rounded-lg item banner" />
          ...
          <section className="flex flex-col gap-2">
            ...
            <EventDetails image="/icons/calendar.svg" alt="Calendar Icon" info={date} />
